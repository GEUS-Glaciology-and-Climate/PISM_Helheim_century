...
**** Make bootstrappable nc file for Greenland (1km)

Workflow:
<< ... >> means that this is the place where code from the code subsection below is run.
...
WORKING=model_input/pism_greenland_1km_v3.nc
...
Set the correct crs for the shape file:
...
rasterize_ftt_mask_to_helheim_nc
...
Code to fill empty data in netcdf file: fill_climate_values.py
#+BEGIN_SRC python :tangle scripts/fill_climate_values.py
import xarray as xr
import sys
...
print(df_filled)
df.to_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec.csv')
df_filled.to_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec_ffill.csv')
#+end_src
...
import pandas as pd
import matplotlib.pyplot as plt
dmi = pd.read_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec.csv', parse_dates = True, index_col = 0)
#print(dmi)
...
dmi_fill.to_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec_filled.csv')
...
df_filled = pd.read_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec_filled.csv', parse_dates = True, index_col = 0)
...
filename = "model_input/forcing/pism_one_station_dmi_tas_noleap.nc"
...
Adding the time bounds required by PISM
#+BEGIN_SRC sh
ncap2 -O -s 'defdim("nv",2);time_bnds=make_bounds(time,$nv,"time_bnds");' \
      model_input/forcing/pism_one_station_dmi_tas_noleap.nc model_input/forcing/pism_one_station_dmi_tas_noleap-with-bounds.nc
...
std_mean.to_csv('model_input/forcing/csvs/air_temp_sd_PROMICE_mean.csv', index = True)
...
dmi = pd.read_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec_filled.csv', parse_dates = True, index_col = 0)
#print(dmi)
...
plt.show()
dmi_cold.to_csv('model_input/forcing/csvs/cold_climate_1776_1896.csv')
#+end_src
...
df_filled = pd.read_csv('model_input/forcing/csvs/cold_climate_1776_1896.csv', parse_dates = True, index_col = 0)
...
# Create the NetCDF file and add dimensions
filename = "model_input/forcing/pism_one_station_tas_cold_noleap.nc"
rootgrp = Dataset(filename, "w", format="NETCDF4")
...
#+BEGIN_SRC sh
ncap2 -O -s 'defdim("nv",2);time_bnds=make_bounds(time,$nv,"time_bnds");' \
      model_input/forcing/pism_one_station_tas_cold_noleap.nc model_input/forcing/pism_one_station_tas_cold_noleap-with-bounds.nc
...
file_path = 'model_input/forcing/pism_one_station_dmi_tas_noleap-with-bounds.nc'
df_filled = pd.read_csv('model_input/forcing/csvs/dmi_tas_historical_temp_prec_filled.csv', parse_dates
                        = True, index_col = 0)
    ...
    try:
        ...
        with xr.open_dataset(modeldir) as DS:
            ...
            for i in range(nx):
                ...
                flux[:, i] = vel[:, i] * thk[:, i] * res  # m²/year * m = m³/year
...
for model in models:
    ...
    try:
        ...
        print(f"⚠️ Failed to process {model}: {e}")
...
shp_path = "shp/calving_fronts_of_interest.shp"
...
with xr.open_dataset(file_path) as ds:
    ...
    var_data = var_data.where(var_data >0.9, np.nan)
 
    var_data.plot(ax=axs, alpha = 0.5,cbar_kwargs={"label": "Model ice thickness (m)"})
    ...
    lines.plot(
    ax=axs,
    column='id',        # attribute to color by
    linewidth=2,
    color='black',
    zorder=5
)
    axs.text(318200,-2579700,'(1)', color = 'black')
    axs.text(311300,-2578500,'(2)', color = 'black')
    axs.text(307000,-2578200,'(3)', color = 'black')
    axs.set_title("")
    axs.set_ylim(-2.6e6,-2.54e6)
    axs.set_xlim(250000,350000)
    
    plt.show()
fig.savefig('figures/paper/map_'+model + '.png')  
    #+END_SRC

    #+RESULTS: plot_thk_w_background
    : None
...
# --- Panel labels (added here) ---
axT.text(0.02, 0.85, "(a)", transform=ax[0].transAxes,
           fontsize=12, fontweight="bold")
axIRD.text(0.02, 0.85, "(b)", transform=ax[1].transAxes,
           fontsize=12, fontweight="bold")
axCF.text(0.02, 0.85, "(c)", transform=ax[2].transAxes,
           fontsize=12, fontweight="bold")
...
axP.set_ylabel('Prec. (mm/yr)')
axT.grid(True, alpha=0.3)
lines = legT + legP
labels = [l.get_label() for l in lines]

# Make one legend on the temperature axis
axT.legend(lines, labels, loc="upper center", ncols=2)

#axT.legend([legT,legP], ['Temp', 'Prec'])
axIRD.plot(composite.index, composite, label = 'Ice rafted debris', color = ird_color)
axIRD.set_ylabel('IRD (g m$^{-2}$ yr$^{-1}$)')
axIRD.legend(loc = 'lower right')
...
#axCF.text(xlim[1] + 40, 3.2, '2021', va='center', fontsize=9, color='gray')
# Reference lines
axCF.plot([autoterm.index[0], temp.index[-1]],[0.8,0.8], color='gray', alpha=0.5, linestyle='--', lw=1)
axCF.plot([autoterm.index[0], temp.index[-1]],[-4.1,-4.1], color='gray', alpha=0.5, linestyle='--', lw=1)
axCF.plot([autoterm.index[0], temp.index[-1]],[3.2,3.2], color='gray', alpha=0.5, linestyle='--', lw=1)

axCF.text(composite.index[35], 0.7, "(1)", color="gray")
axCF.text(composite.index[35], -4.2, "(2)", color="gray")
axCF.text(composite.index[35], 3.1, "(3)", color="gray")

axCF.set_ylabel('Dist. (m)')
#axCF.legend(loc = 'upper left')
legSID, = axSID.plot(flux_obs_annual.index, flux_obs_annual, label = 'Ice flux', color = flux_color)
axSID.set_ylabel('Flux (Gt/yr)')
...
#+RESULTS:
: /tmp/babel-4K0yjp/python-RnotCL:71: SyntaxWarning: invalid escape sequence '\c'
:   axT.set_ylabel('Temp. ($^\circ$ C)')
...
ax[1].axhline(y=0.8, color='gray', alpha=0.5, linestyle='--', linewidth=1)
ax[1].axhline(y=-4.1, color='gray', alpha=0.5, linestyle='--', linewidth=1)
...
ax[1].text(xlim[1] + 40, 0.7, '(2)', va='center', fontsize=9, color='gray')
ax[1].text(xlim[1] + 40, -3.9, '(1)', va='center', fontsize=9, color='gray')
ax[1].text(xlim[1] + 40, 3.1, '(3)', va='center', fontsize=9, color='gray')
...
# ---------------------------------------------------------
<<load_IRD>> # — assumed this sets up a DataFrame called 'composite' with a datetime index
<<load_pism_forcing>>
temp = temp.resample('YE').mean()-273.15
cycle6 = '6th'
...
ax_top.set_ylabel("IRD (g m$^{-2}$ yr$^{-2}$)")
ax_top.grid(True, alpha=0.4)
ax_top.legend(loc='upper left', bbox_to_anchor=(0.1, 1.0))
...
ax_mid.plot(flux_relax.index, flux_relax,
            color='gray', alpha=0.8, lw=1.3, label='7th Reference')

ax_mid.set_ylabel("Flux (Gt yr$^{-1}$)")
ax_mid.set_ylim(10, 40)
ax_mid.grid(True, alpha=0.4)
ax_mid.legend(loc='upper left', bbox_to_anchor=(0.1, 1.2))
...
ax_bot.set_ylabel("Dist. (km)")
ax_bot.set_xlabel("Year")

# Reference lines
ax_bot.axhline(0.8, color='gray', alpha=0.8, linestyle='--', lw=1)
ax_bot.axhline(-4.1, color='gray', alpha=0.8, linestyle='--', lw=1)
ax_bot.axhline(3.2, color='gray', alpha=0.8, linestyle='--', lw=1)

# Move right-side reference years a bit outward
xlim = ax_bot.get_xlim()
right_x = xlim[1] - xlim[1]*0.1 #+ (xlim[1] - xlim[0]) * 0.0

ax_bot.text(right_x, 0.7, "(2)", va="center", color="gray")
ax_bot.text(right_x, -3.9, "(1)", va="center", color="gray")
ax_bot.text(right_x, 3.1, "(3)", va="center", color="gray")
...
#+RESULTS:
: /tmp/babel-4K0yjp/python-cCiQQm:207: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.tight_layout()
...
#ax2.scatter(bjork.index, bjork['HubDist'], color = bjork_color, label = 'Bjørk et al., 2012')
...
ax.set_ylabel('g m$^{⁻2}$ yr$^{⁻2}$', fontsize=11)
...
labels = [ "115 (1st)", "230 (2nd)", "345 (3rd)", "460 (4th)", "575 (5th)","690 (6th)","805 (7th)"]


# Observed thickness (smoothed)
axr.plot(obs.index, obs['surf'].rolling(rolling_window, center=True).mean(), label='Observed 2022', linewidth=2, color='black')
axr.plot(thk81.index, thk81.rolling(rolling_window, center=True).mean(), label='Observed 1981', linewidth=2, color='black', linestyle = '--')
...
for i in range(0,5):
    ...
    axr.plot(model_df.index, smoothed['surf'], alpha = 0.6, color = colors[i], label = labels[i]) #
...
model_df['surf'] = bed['topg']+model_df['thk']
smoothed = model_df.rolling(rolling_window, center=True).mean()
axr.plot(model_df.index, smoothed['surf'], linewidth = 2, color = colors[i], label = labels[i]) #
...
axr.plot(bed['topg'].rolling(rolling_window, center=True).mean(), linewidth = 2, color = 'tab:brown', label = 'Basal topography')
#axr.set_title('Smoothed Surface Profiles Along Centerline')
#axr.set_xlabel('Distance along centerline (km)', fontsize=9)
axr.set_ylabel('Ice surface height (m)', fontsize=9)
...
axr.vlines(21.5, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axr.vlines(27, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axr.vlines(31.0, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axr.text(20.5,ymax-500,'(1)', color = 'gray')
axr.text(26.5,ymax-500,'(2)', color = 'gray')
axr.text(30.5,ymax-500,'(3)', color = 'gray')


# adding the second panel with control runs
...
# Observed thickness (smoothed)
axc.plot(obs.index, obs['surf'].rolling(rolling_window, center=True).mean(), label='Observed 2022', linewidth=2, color='black')
axc.plot(thk81.index, thk81.rolling(rolling_window, center=True).mean(), label='Observed 1981', linewidth=2, color='black', linestyle = '--')
...
for i in range(0,5):
    ...
    model_df['surf'] = bed['topg']+model_df['thk']
    smoothed = model_df.rolling(rolling_window, center=True).mean()
    axc.plot(model_df.index, smoothed['surf'], alpha = 0.6, color = colors[i], label = labels[i]) #
...
model_df['surf'] = bed['topg']+model_df['thk']
smoothed = model_df.rolling(rolling_window, center=True).mean()
axc.plot(model_df.index, smoothed['surf'], linewidth = 2, color = colors[i], label = labels[i]) #
...
axc.plot(bed['topg'].rolling(rolling_window, center=True).mean(), linewidth = 2, color = 'tab:brown', label = 'Basal topography')
#axc.set_title('Smoothed Surface Profiles Along Centerline')
axc.set_xlabel('Distance along centerline (km)', fontsize=9)
axc.set_ylabel('Ice surface height (m)', fontsize=9)
...
axc.vlines(21.5, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axc.vlines(27, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axc.vlines(31.0, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
axc.text(20.5,ymax-500,'(1)', color = 'gray')
axc.text(26.5,ymax-500,'(2)', color = 'gray')
axc.text(30.5,ymax-500,'(3)', color = 'gray')


# --- Panel labels (added here) ---
axr.text(0.02, 0.90, "(a)", transform=ax[0].transAxes,
           fontsize=12, fontweight="bold")
axc.text(0.02, 0.90, "(b)", transform=ax[1].transAxes,
           fontsize=12, fontweight="bold")



plt.tight_layout()
fig.savefig('figures/paper/final_centerline_thk_relax_control_smoothed.png', dpi=300)
plt.show()
...
ax.vlines(21.5, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
ax.vlines(27, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
ax.vlines(31.0, ymin,ymax-550, alpha=0.5, color = 'gray', linestyle = '--')
ax.text(20.5,ymax-500,'(1)', color = 'gray')
ax.text(26.5,ymax-500,'(2)', color = 'gray')
ax.text(30.5,ymax-500,'(3)', color = 'gray')
plt.tight_layout()
plt.show()
fig.savefig('figures/paper/final_centerline_thk_control.png')
